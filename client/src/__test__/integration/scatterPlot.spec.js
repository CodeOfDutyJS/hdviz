...
import '../../model/VisualizationModels/ScatterPlotMatrixModel';

const rootStore = new RootStore();
const { visualizationStore } = rootStore;

describe('#(Integration) Check ScatterPlot Distance Matrix', () => {
  const mockDataset = [
    {
      a: 'first', b: 0, c: 1, d: 2,
    },
  ...
  test('select settings', () => {
    visualizationStore.setVisualizationSelected('scatter');
    visualizationStore.setDistance('euclidean');
    rootStore.modelStore.setFeatures(['b', 'd']);
    rootStore.modelStore.setTargets(['a']);
    expect(visualizationStore._visualization._visualizationSelected.id).toBe('scatter');
